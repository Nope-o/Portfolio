(function renderProjectDetails() {
  ...
  app.innerHTML = `
    ...
    <section class="card like-compact like-strip">
      <div class="like-row">
        <h2 id="like-prompt" class="${alreadyLiked ? "is-hidden" : ""}">Do you like it?</h2>
        <button
          id="like-btn"
          class="action like-emoji-btn ${alreadyLiked ? "secondary" : "primary"} ${alreadyLiked ? "is-hidden" : ""}"
          ${alreadyLiked ? "disabled" : ""}
          title="${alreadyLiked ? "Feedback submitted" : "Submit like"}"
          aria-label="${alreadyLiked ? "Feedback already submitted" : "Submit like"}"
        >
          👍
        </button>
      ...
      <p id="like-status" class="like-status">
        ${alreadyLiked ? "Thank you for liking, it means a lot!" : ""}
      </p>
    </section>
