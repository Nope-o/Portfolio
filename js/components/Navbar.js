...
function Navbar({ activeTab, setActiveTab, isDark, onToggleTheme }) {
  ...
  const [hideMobileBrandStrip, setHideMobileBrandStrip] = React.useState(false);
  const hideMobileBrandStripRef = React.useRef(false);
  const brandRafRef = React.useRef(null);
  ...
  React.useEffect(() => {
    if (!isMobile) {
      hideMobileBrandStripRef.current = false;
      setHideMobileBrandStrip(false);
      return;
    }

    const handleBrandStripScroll = () => {
      if (brandRafRef.current !== null) return;

      brandRafRef.current = requestAnimationFrame(() => {
        const currentScrollY = window.scrollY || 0;
        const deltaY = currentScrollY - lastScrollYRef.current;
        lastScrollYRef.current = currentScrollY;
        let nextHidden = hideMobileBrandStripRef.current;

        if (currentScrollY <= 14) {
          nextHidden = false;
        } else if (deltaY > 3 && currentScrollY > 48) {
          nextHidden = true;
        } else if (deltaY < -3) {
          nextHidden = false;
        }

        if (nextHidden !== hideMobileBrandStripRef.current) {
          hideMobileBrandStripRef.current = nextHidden;
          setHideMobileBrandStrip(nextHidden);
        }
    ...
    window.addEventListener('scroll', handleBrandStripScroll, { passive: true });

    return () => {
      window.removeEventListener('scroll', handleBrandStripScroll);
      if (brandRafRef.current !== null) {
        cancelAnimationFrame(brandRafRef.current);
        brandRafRef.current = null;
      }
    };
  }, [isMobile]);
  ...
  const headerClasses = `header-glass backdrop-blur shadow sticky top-0 z-50 transition-all duration-300 ease-in-out ${
  ...
  const mobileBrandStripClasses = `mobile-brand-strip ${hideMobileBrandStrip ? 'mobile-brand-strip-hidden' : 'mobile-brand-strip-visible'}`;
  ...
  return (
    <>
      ...
      {isMobile && (
        <div className={mobileBrandStripClasses} aria-hidden={hideMobileBrandStrip}>
          <div className="container mx-auto px-4 md:px-8">
            <button
              type="button"
              onClick={handleLogoClick}
              className={`mobile-brand-strip-logo ${isDark ? 'text-slate-100' : 'text-slate-900'}`}
              aria-label="Go to About section"
            >
              <span className="mobile-brand-strip-text">Madhav Kataria</span>
            </button>
          </div>
        </div>
      )}
