  ...
  React.useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      rafIdRef.current = requestAnimationFrame(() => {
        ...
        const compactOn = SCROLL_DOWN_THRESHOLD + 10;
        const expandOn = Math.max(0, SCROLL_UP_THRESHOLD - 10);
        let nextIsCompact = isScrolledDownRef.current;

        // Hysteresis: separate enter/exit points to prevent rapid toggling near threshold.
        if (currentScrollY > compactOn) {
          nextIsCompact = true;
        } else if (currentScrollY < expandOn) {
          nextIsCompact = false;
        }
      ...
      if (!window.scrollY && isScrolledDownRef.current) {
        isScrolledDownRef.current = false;
        setIsScrolledDown(false);
      }
    };
